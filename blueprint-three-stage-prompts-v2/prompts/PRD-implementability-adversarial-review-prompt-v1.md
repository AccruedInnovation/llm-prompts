...
**Primary contract:** `PRD-implementability-authoring-prompt-v2.md`, coordinated version 2, set revision `coordinated-r1`, dated 2026-09-04  
...
This companion adds review methods, finding conventions, and a repair procedure. It does not retroactively add product requirements to the authoring prompt. Cite the governing obligation behind each conformance finding; label other improvements as proposals. Use the exact supplied authoring prompt: the coordinated v2 replaces an earlier standalone v2 with a similar filename. References below to numbered sections mean this review prompt unless marked **PRD §**.
...
First record an evidence-backed assessment of the unchanged candidate. Inspect its claims, artifacts, meaningful interactions, and worker handoffs before rewriting them. Do not silently repair the evidence for a defect before preserving what failed. Keep review observations and later repair authorship distinct.

Use a small review plan covering every required dimension and slice, with deeper checks on material uncertainty, changed interfaces, state/effects, acceptance, bootstrap, and prior failures. One trace can cover several related concerns. Do not impose a finding quota, reviewer count, formal model, or tool suite. Record unreviewed scope instead of treating a sample as complete coverage.

Investigate material concerns using authorized inspection, calculations, contract examples, disposable probes, and current evidence. Distinguish an executed result from an analytical trace, specification gap, or untested concern. Test the strongest reasonable rebuttal before retaining a major finding. Use sections 18 and 21 for concrete probes and finding discipline.

Use genuine independent agents, sessions, or human reviewers when available and useful, and whenever a governing review requirement needs them. Give each a bounded, read-only subject; exact baseline; allowed sources/access/effects; relevant assertions; and expected evidence. Use one writer per canonical surface and integrate returned findings yourself. Parallelize independent analysis, not incompatible private changes to a shared contract.

Shell subprocesses, parallel tests, separate headings, role-play, and later self-review in the same context are not independent reviewers. Record actual capability and exposure. Schedule a fresh worker-consumption pass before exposing that recipient to author-only context. If you later repair the packet, reassess the independence and currentness of affected review evidence rather than counting your own re-reading as fresh review.

Run repair and recheck cycles while they resolve material findings within authority. Continue useful independent work when another branch blocks. Stop when applicable obligations are satisfied, only supported unresolved gates remain, or a real execution/resource limit ends the run. Preserve partial artifacts, remaining scope, and the next required action; do not promise background completion or reduce scope to claim success.
...
These are inherited architecture-assignment semantics, not a demand for another mandatory manifest. For a direct assignment, establish equivalent necessary scope and authority without fabricating an upstream package.
...
Verify exact caller/callee signatures, preconditions, postconditions, errors and their precedence, retryability, timeouts, cancellation, idempotency, resource limits, and effects prohibited on rejection. Check serialization, identity/hashing, timestamps and clocks, path rules, locale, and deterministic ordering only where the actual boundary needs them.
...
For PRD §11, inspect every worker-facing entry point against the actual target profile. One recipient may cover several slices, but one successful sample does not establish unreviewed slices. A later slice may receive a complete conditional review without pretending its future inputs exist.

Each slice must identify, directly or by exact accessible reference:

- Its ID, parent assignment, allocated obligation/contribution, concrete outcome, responsible role, dependencies, and sequencing reason.
...
- Reading order, required sections/contracts/fixtures, source and intended target paths, baseline, and tool/environment profile.
...
Check context and reasoning fit. Include required reading, tool results, working space, and output needs—not only the length of the slice description. Use known capability evidence when supplied. When capacity is unknown, state the minimum needed and a gate to check it; do not invent a model context limit or available tool. Raw word counts are estimates, not measured model capacity. Worker-suitability uncertainty is material when the design depends on it.
...
Check one writer per mutable surface, shared-file integration ownership, resource conflicts, and dependencies before parallel work. A role may be late-bound to a worker; ownership and authority semantics may not. Every split introduces integration and verification work that must have an owner.
...
Set equality is necessary, not sufficient. Inspect whether each linked contract, test, expected result, and gate proves the actual obligation or contribution. A row containing all the right IDs may still test a stub, a different subject, or only schema shape. Planned evidence may complete the design trace only when its acceptance semantics, production phase, and usable method are defined; it is not observed proof.
...
Before a PRD can claim implementation readiness, each slice needs fresh-context recipient review using only the material and access its implementation worker will receive. A separate agent, session, or human may perform the review; one reviewer may cover multiple slices. No new call for each file or permanent reviewer service is required.

First inspect existing review evidence. Check reviewer identity/role, actual context and access, exact packet/baseline, slice coverage, substantive responses, defects, repairs, disposition, and currentness. Preserve valid evidence for unchanged material. A review title, signature, “looks complete,” or a completion message without evidence-bearing consumption answers is insufficient.

A separately invoked adversarial reviewer may also serve as a fresh recipient **only if it first consumes the slice from the actual worker packet and access before seeing author-only design context or solutions**. Record that boundary and the recipient's answers at the time. Broad source review followed by an instruction to forget the history is not a fresh-context test. Access to prior findings or coaching must be disclosed where it limits a claimed blind or unaided property.

Give the recipient this bounded assignment for each slice, allowing exact canonical references rather than duplicated content:

1. State the concrete outcome, allocated obligations/contribution, target maturity, operating limits, and relevant baseline.
2. Name the first substantive actions and locate each input, contract, fixture, command, and expected output needed for them.
3. Distinguish existing/supplied inputs, accepted-predecessor conditions, current-slice outputs, and late-bound values; identify what exists now and what cannot yet be used.
4. Explain required normal, error, state/effect, recovery, and rejection behavior, including prohibited effects and precedence where relevant.
5. Identify the exact checks, acceptance rules, expected evidence, exit gate, downstream consumer, and local versus integrated acceptance owner.
6. State the local choices permitted, the change that would exceed authority, and the usable escalation route.
7. Assess reading/access/context needs, missing prerequisites, conflicting instructions, and any design or test standard the worker would have to invent.
8. State entry/dispatch status with reasons and the changes that would invalidate the packet or its evidence.

Compare answers against the authoritative packet. Record actual lookup paths, material answers, errors, guesses, contradictions, missing references, author interventions, and coverage. Do not demand a full implementation to prove document consumption. For a later slice, review its complete conditional handoff without fabricating predecessor outputs or declaring it executable now.

Repair canonical material, derive the affected set, and repeat affected consumption checks. A reviewer that authored a substantive repair is not fresh with respect to that repair merely by changing roles. Use another genuinely fresh recipient for affected material when required, or retain a visible missing-review gap. Reuse unaffected review evidence with its exact binding and reason; editorial changes with no effect on consumption do not automatically require a full new review of every slice.

When no fresh recipient is available and no valid current evidence supplies the required review, complete an author-only consumption check, label it accurately, and keep the required review row `PARTIAL`. Do not claim `PASS` through stronger self-review, deterministic checks, role-play, or this adversarial review's existence. If a material unknown about worker suitability could change the design, classify that separate design-critical gap under section 23 rather than hiding it as a routine missing review.

### Bounded target-worker trial

Require a trial when the assignment's review policy calls for it or worker suitability remains a material unresolved assumption. Otherwise record whether a trial ran and limit claims accordingly; a larger reviewer understanding the document is not evidence that the intended smaller worker implemented it successfully.

A trial needs separate implementation/effect permission and a genuinely dispatch-ready scope under the applicable approvals. It cannot waive the very gate it tests. When formal PRD readiness is incomplete, only a separately authorized, independently complete trial scope with its own boundaries and gates may execute; do not relabel the incomplete PRD to bypass them.

Use a representative slice that exercises the relevant reasoning or integration, not merely trivial edits. Fix baseline, exact packet, worker profile, tools, allowed effects, acceptance semantics, and permitted interventions before the run. Record actual execution, results, missing prerequisites, questions, interventions, rework, and candidate binding. Author rescue or changed expected results is not unaided worker success. An unrun trial is `NOT_RUN`, not a simulated success; its readiness effect follows the policy and the unresolved claim.
...
- **Consequence and reach:** named outcome, risk, contract, avoidable cost, affected slices/PRDs, and precise design/approval/dispatch/acceptance claim or gate.
...
Derive the full affected set using the actual changes and canonical dependencies. Recheck relevant schemas/examples, semantic cases, bootstrap, reference availability, source/install paths, ownership, traceability, recipient consumption, and statuses. Changes to shared semantics or allocation need the proper owner and successor upstream record. Hold dependent commitments while preserving independent useful work.
...
Use only `PASS`, `PARTIAL`, or `BLOCKED` for each of the 21 PRD matrix rows. Each row needs evidence/section and any remaining gap/owner. A genuinely inapplicable dimension uses `PASS` with `None —` and a scoped reason; this is not a runtime test result.
...
- **`PASS`** only when every row passes; material decisions are resolved within authority; required author-supplied artifacts and author-time checks/reviews are complete; slices have defined owners, contracts, and gates; and no unresolved assumption could change the design. Planned predecessors are permitted only with complete contracts, production steps, acceptance criteria, and a non-circular path to availability.
- **`PARTIAL`** when useful design exists but authoring, artifact validation, or required review remains incomplete and no row is blocked. Missing fresh-recipient evidence is not a fabricated pass.
...
A review can be complete while the candidate is blocked. Report review coverage and limits separately. When required sources prevent your assessment, retain the candidate's stated status as an **unverified claim**, not your own verdict. Do not issue a revised `PASS` without sufficient current supporting evidence. State whether runtime behavior, intended-worker performance, independent review, or integration acceptance remains unproved.
...
Use the real packet from the recipient's delivery root. A valid path in the author's workspace may fail after handoff. Where a transferable archive is requested, check safe paths and contents, extract into a separate location, verify relative references and artifact identity, and provide a checksum. Do not claim install/runtime behavior, reproducibility, offline use, another platform, or successful importer use from extraction alone.

Keep candidate inventory and review evidence acyclic: identify subject bytes separately from reports that refer to their digest; use a detached outer archive checksum rather than requiring a manifest to hash itself. A simple identity mechanism is sufficient when it accurately binds the claims. Do not invent historical diffs or inaccessible download paths.
...
4. **Design, artifact, and coverage audit.** All 13 PRD sections and 21 matrix dimensions assessed; artifact/reference availability; `U`, `L`, `O`, complete mapping and set differences; semantic proof gaps; integration and final acceptance owners. Reference canonical tables instead of copying independently editable inventories.
5. **Recipient and worker evidence.** Per-slice consumption coverage, reviewer/context/baseline, substantive answers and defects, preserved or refreshed evidence, trials actually run, interventions, missing prerequisites, and limits on freshness or target-worker claims.
...
Retain complete content or exact accessible authoritative references for all applicable obligations; use scoped `None —` reasons for irrelevant concerns. The main PRD ends with its matrix, `Implementation-ready: yes` or `Implementation-ready: no`, decisive reasons, approval/gates, separate dispatch summary, and unestablished claims. Keep review/validation evidence in earlier sections or supporting files so the prescribed ending remains clear.
...
Return the concise authoring result to the architecture/integration owner within the PRD or handoff: PRD identity/baseline, upstream allocations/contribution coverage, justified local requirements, decisions within delegation, shared/architecture findings and dispositions, acceptance ownership, readiness/approval/dispatch, actual artifacts, and evidence. No separate mandatory authoring-result file is needed.
...
- Did you review the actual output against the correct coordinated v2, original assignment, current architecture, and repository/new-project evidence rather than an imagined baseline?
...
- Did you assess every slice's packet/access/context, conditional dependencies, first actions, ownership, and gates without inventing worker capacity or predecessor outputs?
- Did you check all 12 mandatory source self-review questions and the recipient/trial policy, with exact coverage and truthful independence, currentness, and intervention records?
...
[Exact coordinated PRD-authoring prompt; architecture assignment/baseline, canonical contracts and decisions, requirement allocation and contribution boundaries; source precedence; repository and local changes or confirmed new-project state; accepted predecessor outputs and evidence; applicable project guidance and adoption status.]

## Review mode, scope, and priority

[`REVIEW_AND_REVISE` by default, or `REVIEW_ONLY`; PRDs/slices under review; required full coverage; specific concerns; authorized resource limits without dropping unreviewed scope silently.]
