...
Do not renumber published IDs. Retain planner B-/D- IDs and earlier package IDs as aliases when introducing BLK-/ADR-/PRD- identities; publish explicit mappings. Mark retired objects as superseded rather than reusing identity.
...
- **Responsibility view** — systems, architecture elements, ownership, and authority.
...
Where quality drives the architecture, allocate relevant budgets across the actual dependency path and check feasibility with proportionate calculation, existing evidence, or an experiment. Account for contention, retries, retained state, operating load, and failure cases as relevant. State assumptions and sensitivity. A numerical target alone is not a performance or capacity argument.
...
Organize implementation around **integrated capability increments**, not repository boundaries or horizontal technical layers.
...
Translate capability increments into coherent PRD scopes, not worker tasks. One small system may need one PRD; a large system may need several. Do not split automatically by repository, component, or horizontal layer. Each scope must lead to a verifiable outcome or a necessary foundation with a named integrated successor.

For each `PRD-` assignment, supply a concise entry point containing:

- **Identity and baseline:** assignment ID, package ID/revision, source brief refs, controlling contracts and revisions, and currentness rules.
- **Outcome and allocation:** capability IDs, exact outcome, in/out-of-scope work, allocated requirements and invariants, contributing obligations, and final acceptance owner.
...
- **Authoring environment:** available repositories or known absence, source baseline, access, tools, output location, allowed/forbidden effects, and applicable review policy. This describes authoring, not permission to execute worker slices.
...
Check that the union of allocated active requirement IDs covers the committed architecture set, with no unapproved additions, missing IDs, or duplicate definitions. Many-to-many references are valid. For a requirement spanning PRDs, define each contribution, the integration dependency, and one final acceptance owner. Separate component passes do not establish the whole requirement.
...
- architect for each genuinely distinct system;
...
When genuine independence is unavailable, perform explicitly separated review passes and disclose that they are self-review. Compensate with deterministic cross-reference checks, exact candidate binding, contradiction searches, and fresh re-review after repairs.
...
Require information, not a large file tree. Choose the smallest layout that gives the recipient a usable authoritative baseline. Reuse project formats and existing tools. Do not build a new documentation framework to satisfy this prompt.
...
<package-root>/
  ...
  03-architecture/                         # baseline, responsibilities, state/data, deployment, systems
...
The human entry point must state identity/revision, purpose, target maturity and exposure, package completeness, approval, systems and shared boundaries, controlling records, per-PRD readiness, unresolved items, reading order, and what the package does not authorize. It must identify the source-of-truth location for each class of obligation and which files are projections.

The inventory or manifest must list artifacts, roles, status, IDs, canonical-versus-projection role, relevant source/revision bindings, and availability. Include hashes where required or useful. Do not maintain different editable meanings in the ledger, ADRs, interface files, PRD briefs, and YAML. Validate projections against their canonical records.
...
Use actual assignment values; remove empty objects only when genuinely inapplicable and explain required non-applicability. Each dependency needs type, source/producer, contract or decision ref, required-availability phase, and gate. Each fixed reference needs identity, location, controlling revision, and authority. Each delegated decision needs its boundary and owner. Section 22 governs their semantics whether the handoff is YAML or prose.

When an archive is requested, validate the unpacked contents, verify extraction and relative paths, and provide its checksum. Do not fabricate artifacts, validation, or a successful import.
...
Use proportionate calculations, traces, models, or concrete counterexamples to test central claims. Reuse current valid evidence. Resolve contradictions or lower readiness for affected scopes; do not hide them behind individually plausible component descriptions.

### PRD-author consumption review

Check whether a recipient can identify the assigned outcome, architecture baseline, fixed decisions, permitted design choices, shared contracts and owners, dependencies, required elaboration, acceptance obligations, and escalation route using only the delivered package and declared access.
...
- `COMPLETE`: the package satisfies applicable architecture and handoff obligations for its declared scope, including required author-time checks. Future PRD elaboration and future runtime qualification remain assigned, not falsely completed.
- `PARTIAL`: useful content exists, but architecture authoring, supplied-material validation, or required review remains incomplete without a design-critical blocker.
...
- `READY_FOR_PRD` means its system-wide obligations are coherent and settled under valid authority; required authoring inputs and canonical definitions exist; the recipient can complete the remaining bounded detailed design without recovering missing intent or inventing shared behavior; and required architecture-stage checks have passed.
...
Summarize ready, partial, blocked, and unassessed scopes separately. A blocked later capability does not erase readiness of an independent earlier scope. Bind all claims to the reviewed package and contract revisions. Never label this architecture package implementation-ready or `READY_FOR_EXECUTION`.
...
Summarize the outcome, participating systems, main responsibilities, shared architecture, operating limits, and what was intentionally excluded.
...
Identify assigned PRD scopes, their outcomes, fixed obligations, permitted detailed design, dependencies, integration and final acceptance owners, and the exact entry point for each author. Do not supply worker dispatch claims.

### 7. Verification and Readiness Evidence
State structural checks, whole-system review, recipient review actually performed, results, repairs, and limits. Separate future implementation evidence from existing observations.
...
[Current design_brief_id/revision, architecture_assignment, decision_record, evidence_refs, fixed_decision_refs, open_items, artifact_inventory, and approval/readiness. Omit when no planner output exists; do not invent an upstream stage.]
...
[One or more desired PRD outcomes; known allocation and integration owners; whether candidate authoring is permitted; required architecture/recipient reviews; approval gates. Discover or propose missing bounded scope within authority.]
...
[List available document tools, code inspection tools, VCS state, offline packages, genuine sub-agent capability, and required domain or language profiles.]
