...
**Revision:** 3.0 — PRD-bound execution, gated slices, protected acceptance, and exact-candidate delivery

**Companion:** Coordinated three-stage prompts v2, set revision `coordinated-r1`  
**Status:** Review draft · 2026-09-04
...
Create an exact final candidate inventory before final qualification regardless of VCS. Record the actual setup: existing Git/`jj`, ephemeral Git, Beads, hashes, or a combination. Include relevant commits/change IDs, local changes, manifests, and actual pre-change evidence. A commit omitting uncommitted inputs does not identify the full candidate. Never claim historical diff coverage that was not captured.

Record created, modified, replaced, and removed files; relevant hashes; canonical sources and generators; toolchain/environment; dependencies/locks; migrations; verification commands; retained scaffolding; and cleanup disposition. Modify canonical sources and regenerate projections unless the authorized task requires otherwise.

Finish source-changing documentation, regeneration, and cleanup before the final freeze where possible. Any later repair, packaging change, or subject-changing cleanup refreshes the candidate identity and invalidates affected checks. Non-subject cleanup need not trigger unrelated checks, but record its boundary. Do not attach stale verification to the final subject.

For an archive, installer, package, export, or other transferable deliverable, verify paths, contents, structure, extraction/opening, and digest. Test the required install or consumer path from that **actual artifact** in the specified clean environment, not only the authoring workspace. Use a separately identified disposable extraction/installation so the tested delivery artifact remains exact. Do not claim reproducibility, another platform, hardware qualification, or offline behavior from extraction alone.

Keep secrets and restricted data out of reports, logs, inventories, and continuation records. Use approved secure evidence locations and useful redacted references without exposing credentials.

Keep an acyclic inventory: identify the implementation/package candidate separately from result records and logs that refer to its digest. The outer handoff archive may have a detached checksum; do not require a manifest to hash itself. Qualify any required outer artifact behavior and report exactly which bytes each check covered.

Preserve required evidence and continuation records before deleting their source store. Remove task-owned temporary processes, sockets, caches, credentials, workspaces, VCS/Beads metadata, and fixtures only within permission and retention rules. Exclude execution-only metadata from product delivery unless requested. Report incomplete cleanup and retained resources without deleting unrelated work or the only recoverable partial candidate.
...
Provide accessible deliverables and created/modified/replaced/removed inventories; source and package digests; actual tool/VCS/work-graph use; useful diffs/commits and local-change coverage; and execution metadata intentionally excluded. Do not invent download paths or historical evidence.
