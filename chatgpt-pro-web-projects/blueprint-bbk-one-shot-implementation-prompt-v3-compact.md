...
**Edition:** Compact — standalone; same authority, gates, and result rules as the full v3 prompt  
...
Freeze an exact inventory before final qualification. Record actual VCS/Beads/hash setup, commits/change IDs, local changes, pre-change evidence, created/modified/replaced/removed files, digests, canonical sources/generators, material toolchain/dependencies, migrations, commands, retained scaffolding, and cleanup. No invented full historical diff. A commit alone may omit candidate inputs. Modify canonical sources and regenerate projections unless instructed otherwise.

Finish subject-changing docs, regeneration, and cleanup before freeze where possible. Any later change, repair, or packaging/cleanup mutation refreshes identity and invalidates affected evidence. Record non-subject cleanup boundaries; no unrelated reruns are needed for changes proved irrelevant.

Validate actual delivery archive/installer/package/export structure, paths, contents, extraction/opening, and digest; run the required install/consumer checks from that artifact in the specified clean environment, using a separately identified disposable extraction. Workspace tests or successful extraction alone do not prove installed behavior, reproducibility, another platform, hardware, or offline use.
...
Keep inventory acyclic: distinguish implementation/package digests from logs/results that reference them; use a detached checksum for an outer handoff archive rather than requiring self-hashing. Qualify required outer behavior and identify each tested subject.

Preserve required evidence/continuation before deleting its store. Remove only task-owned temporary processes, sockets, caches, credentials, workspaces, metadata, and fixtures within permissions/retention. Exclude execution-only state unless requested; disclose incomplete cleanup. Do not delete user work or the only recoverable partial candidate.
